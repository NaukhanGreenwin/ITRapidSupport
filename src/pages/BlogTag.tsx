...
const BlogTag: React.FC = () => {
  ...
  const [decodedTag, setDecodedTag] = useState('');
  
  useEffect(() => {
    if (tag) {
      // Convert URL format to human-readable
      const formattedTag = tag.replace(/-/g, ' ');
      setDecodedTag(formattedTag);
      
      // Filter posts by tag (case insensitive)
      const filteredPosts = allBlogPosts.filter(post => 
        post.tags.some((t: string) => t.toLowerCase() === formattedTag.toLowerCase())
      );
  ...
  return (
    <PageTransition>
      <SEO 
        title={`Articles tagged with ${decodedTag} | IT Rapid Support`}
        description={`Read our articles about ${decodedTag} and learn more about cybersecurity best practices.`}
      />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="text-center">
            ...
            <p className="text-slate-300 text-lg max-w-2xl mx-auto">
              Browse our articles about {decodedTag} and learn more about cybersecurity best practices.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {posts.length > 0 ? (
            ...
            <div className="text-center py-12">
              <h3 className="text-xl text-gray-700 mb-4">No articles found with this tag</h3>
